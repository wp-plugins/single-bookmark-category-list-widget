...
Description: List bookmarks from one bookmark category for WordPress 2.3+. Licensed under <a href="http://www.gnu.org/licenses/gpl.txt">GPL v2</a>.
Author: Azmeen (HTNet)
Version: 2.0
...
2.0	[28 Sep 2007]:	Branched a WP2.3 Compatible Version
*/
...
function widget_sclw_init()
{
	...
	function widget_sclw($args, $number = 1) {
		...
		$term = get_object_vars($wpdb->get_row("SELECT name FROM $wpdb->terms WHERE term_id=$catid LIMIT 1"));

		echo $before_title . $term['name'] . $after_title;
		echo "<ul>\n";
